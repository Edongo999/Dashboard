...
const Sidebar: React.FC<SidebarProps> = ({ collapsed, setCollapsed }) => {
  ...
  return (
    <>
      ...
      <aside
        className={`hidden lg:flex flex-col h-screen bg-[#656A74] text-neutral-content p-4 transition-all duration-300 ${
          collapsed ? "w-0 overflow-hidden" : "w-64"
        }`}
      >
        {/* ✅ Bouton flottant pour replier/déplier */}
        <button
          onClick={() => setCollapsed(!collapsed)}
          className="absolute top-4 -right-4 bg-white text-gray-800 rounded-full p-1 shadow-md z-50"
        >
